...
- Scope constructors validate canonical repository-relative `{ path, bytes }` snapshots and exact caller-supplied documentation allowlists. The action boundary supplies complete committed and dirty Git scope and excludes prompts, skills, specs, configuration/workflows, and executable instruction Markdown from its docs-only allowlist; model handoffs never authorize repository paths.
