...
- Terminal semantic recovery (`applyTerminalCorrection`) supersedes an active final receipt only after a compare-and-swap on its exact run id, an unambiguous earlier `onFail` reset target inferred from the receipt's compiled code-review stage, and a scope/HEAD eligibility proof; one atomic frozen save archives the exact old receipt with a one-based append-only `sequence`, clears active approval, resets the target plus downstream to pending with the reason carried into the target's findings, and preserves all prior history, attempts, regressions, and economics. Retries with the same expected run id and reason against an authority-free, already-applied `pending`, `running`, `failed`, or `needs-attention` state resume without re-archiving; active checkpoint/receipt authority and all other statuses reject. The kind literal and expected-run-id/body bounds are validated at the core boundary before any effect.

## Gotchas

- Resume normally starts at the first incomplete compiled stage. Before that selection, only a valid legacy nonterminal state below the fixed receipt-introduction version 2, with a durably passed code-review stage and no checkpoint/receipt, persists a budget-neutral reset of review plus downstream and reruns those stages; malformed, newer, version-2+, terminal, and checkpoint-bearing states do not enter this legacy recovery.
