...
Terminal semantic recovery reopens a `done` story through one all-or-none option group on `run`: `--terminal-recovery-kind supersede-contract-invalid-candidate`, `--expected-receipt-run-id RUN_ID`, and `--recovery-reason REASON`. **The sanctioned operator route is the orchestrator's structured `run_pipeline` holding the landing slot for the entire supervisor child lifetime; this package's bare CLI is a low-level implementation/testing seam only and must not be operator-invoked, and it does not implement or acquire the landing slot.** It is local control-plane authentication under the per-story dispatch lock (OS/process authority plus an exact authenticated receipt run identity), never a secret or human credential. Recovery eligibility is the first permitted state write: a missing state rejects without initialization, no reconciliation is persisted before eligibility/compare-and-swap, and stale-ID, drift, committed `HEAD`, invalid kind, reconciliation-needed state, or any state other than an authority-free exact-tail `pending`, `running`, `failed`, or `needs-attention` retry never saves or spawns. The reset target is inferred from the active receipt's quality review stage and that compiled stage's earlier `onFail` target; the supervisor never accepts a caller-selected stage, spec path, state path, or receipt bytes. The first safe version requires `HEAD == finalScopeReceipt.baseOid` and a current branch/base/scope exactly equal to the receipt. One atomic save archives the deeply frozen superseded receipt plus its canonical RunDef identity, clears active approval, and resets the target plus downstream to pending while preserving all earlier stages, histories, attempts, regressions, and economics. Initial `done` eligibility keeps requiring the exact digest. A matching superseded-tail retry in `pending`, `running`, `failed`, or `needs-attention` with no active checkpoint or receipt may adopt a changed digest only when canonical RunDefs become byte-equal after stripping every stage `timeout`; its first transition save atomically persists the new digest and identity. Every other RunDef change retains `state-identity-mismatch`. The same expected run id and normalized reason after a crash resume without a duplicate archive; stale or divergent recovery rejects before any state write or stage spawn. Ordinary runs and zero-stage reattestation stay unchanged; `done` still requires one active final receipt, and archived receipts are history, never active landing authority.
